...
### Proposed Change
`.ease-sr-only` in `core/utilities.css` uses the deprecated `clip: rect(0,0,0,0)` property alongside the modern `clip-path: inset(50%)`. The `clip` property is dead weight and may be removed from future browser specs.
```css
