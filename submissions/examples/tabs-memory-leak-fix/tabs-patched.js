(function () {
  ...
  var isResizeListenerAdded = false;
  ...
  function initializeTabs() {
    ...
    // Add event listeners for dynamic underline width adjustment (Only attach once!)
    if (!isResizeListenerAdded) {
      var resizeTimeout;
      window.addEventListener('resize', function () {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(function () {
          var allContainers = document.querySelectorAll('.ease-tabs');
          Array.prototype.forEach.call(allContainers, function (container) {
            updateUnderlineWidth(container);
          });
        }, 150);
      });
    ...
    Array.prototype.forEach.call(tabContainers, function (container) {
      // Use a class to mark already attached listeners to prevent duplicates
      if (!container.classList.contains('ease-tabs-initialized')) {
        var inputs = container.querySelectorAll('.ease-tab-input');
        ...
        container.classList.add('ease-tabs-initialized');
      }
    });
  }
